...
export default function HeroSection() {
  ...
  const slides = [
    {
      image:
        "https://upload.wikimedia.org/wikipedia/commons/thumb/4/4e/SMA_Negeri_1_Yogyakarta%2C_2024.jpg/1280px-SMA_Negeri_1_Yogyakarta%2C_2024.jpg",
    ...
    >
      ...
      <nav className="relative z-20 flex items-center justify-between p-6 md:p-8">
        ...
        <div className="hidden md:flex items-center space-x-8">
          ...
          >
            <button className="relative text-white hover:text-gray-300 transition-colors duration-300 font-medium tracking-wide pb-1 group flex items-center gap-1">
              Kelola Akun
              <ChevronDown
                size={16}
                className={`transition-transform duration-300 ${isAccountDropdownOpen ? "rotate-180" : ""}`}
              />
              <span className="absolute bottom-0 left-0 w-0 h-0.5 bg-white transition-all duration-300 ease-out group-hover:w-full"></span>
            </button>
            ...
            {isAccountDropdownOpen && (
              <div className="absolute top-full right-0 mt-2 w-64 bg-white rounded-lg shadow-2xl border border-gray-200 overflow-hidden">
      ...
      {isMenuOpen && (
        <div className="absolute top-0 left-0 w-full h-full bg-black/90 z-30 md:hidden overflow-y-auto">
          <div className="flex flex-col items-center justify-center min-h-full space-y-6 py-20">
            ...
            <div className="pt-4 border-t border-white/20 w-64">
              <div className="text-white/60 text-sm font-semibold tracking-wider mb-4 text-center">
                KELOLA AKUN
              </div>
              {accountManagementItems.map((item) => (
                <Link
                  key={item.name}
                  href={item.href}
                  className="block text-center py-3 mb-2 bg-white/10 hover:bg-white/20 rounded-lg transition-colors"
                  onClick={() => setIsMenuOpen(false)}
                >
